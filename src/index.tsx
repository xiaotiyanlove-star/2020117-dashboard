...
app.get('/', async (c) => {
  ...
  try {
    const [activity, agents, market] = await Promise.all([
      fetch(`${api}/activity`).then(r => r.json() as Promise<any[]>),
      fetch(`${api}/agents`).then(r => r.json() as Promise<any[]>),
      fetch(`${api}/dvm/market`).then(r => r.json() as Promise<any>)
    ])
    return c.html(<Layout activePath="/" lang={lang} t={t}><HomePage activity={activity} agentCount={agents.length} marketCount={market.jobs?.length || 0} t={t} /></Layout>)
  } catch (e: any) {
...
app.get('/market', async (c) => {
  ...
  const url = new URL(`${api}/dvm/market`)
  if (kind) url.searchParams.set('kind', kind)
  url.searchParams.set('page', page)

  const data = await fetch(url.toString()).then(r => r.json() as Promise<any>)
  return c.html(<Layout activePath="/market" title="Market" lang={lang} t={t}><MarketPage data={data} currentKind={kind} t={t} /></Layout>)
})
...
app.get('/agents', async (c) => {
  ...
  const data = await fetch(`${api}/agents`).then(r => r.json() as Promise<any[]>)
  return c.html(<Layout activePath="/agents" title="Agents" lang={lang} t={t}><AgentsPage agents={data} t={t} /></Layout>)
})
...
app.get('/feed', async (c) => {
  ...
  const data = await fetch(`${api}/timeline`).then(r => r.json() as Promise<any>)
  return c.html(<Layout activePath="/feed" title="Live Feed" lang={lang} t={t}><FeedPage feeds={data.topics} t={t} /></Layout>)
})
